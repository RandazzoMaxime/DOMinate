...
Same protocol as the test loop: Chromium screenshot of the live HTML, then
DOMinate PDF rasterized at 96 DPI, then `pixelmatch` (threshold 0.1). Source
HTML stays local and is not published.

| | Invoice | SOW (4 pages) |
|---|---:|---:|
| Worst page pixel-diff vs HTML | 0.432% | 1.714% |
| Pages dispatched | 1 | 4 |
| Convert (one call) | 16 ms warm | 32 ms warm |

### Invoice — HTML vs DOMinate PDF

| HTML in Chromium | PDF generated by DOMinate |
|:---:|:---:|
| ![DOMinate invoice rendered as HTML](docs/assets/example-invoice-html.png) | ![The same invoice converted to PDF by DOMinate](docs/assets/example-invoice-pdf.png) |

### Statement of Work — auto page dispatch

A four-page A4 professional document. DOMinate cuts on `page-break` markers and
will not split a text line across a page. Each PDF page is pixel-diffed against
the matching HTML band.

![Four SOW pages dispatched by DOMinate](docs/assets/example-report-pages.png)

| HTML page 1 | PDF page 1 |
|:---:|:---:|
| ![SOW HTML page 1](docs/assets/example-report-html.png) | ![SOW PDF page 1](docs/assets/example-report-pdf.png) |
...
|---|---:|---:|---:|---:|
| **DOMinate** | 16 ms | **0.432%** | 34 ms | **1.714%** |
| html2canvas + jsPDF | 93 ms | 68.7% | 124 ms | 73.8% |
| Chromium `page.pdf()` | 4.9 ms | 0.732% | 7.5 ms | 2.077% |
